2026.08.27
- IPv6 前缀选择兼容 PVE 上联 /128 与委派 /38 并存、SIT/6in4 隧道及 /120、/127 等窄前缀，避免错误把可用地址池遮蔽
- CNI 允许使用不含宿主地址的父前缀子网并安装更精确路由，保留 PVE 委派前缀和常规 /64 的公网独立 IPv6；仅真实路由冲突或无法安全切分时使用隔离 ULA NAT66，已有其他 CNI 配置会原样保留
- NDP 仅在以太网上联需要；NAT66 和非以太网隧道不再因 responder 未运行而让容器回退 IPv4，卸载会清理对应状态文件
- IPv6 回归测试兼容 GitHub Ubuntu 的 ShellCheck 0.9，避免后置测试 mock 被误判为真实命令覆盖
